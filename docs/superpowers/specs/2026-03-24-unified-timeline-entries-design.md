...
// System events (participant_joined, participant_left, topic_changed).
// No authorId — these are generated by the system, not by a participant.
// Always public; no target or recipientId fields.
// SystemEventEntry rows ARE included in agent context (agents see join/leave/topic events).
...
```typescript
function systemEventContent(entry: SystemEventEntry): string {
  switch (entry.type) {
    case 'participant_joined': return `${entry.participantName} joined the chat`
    case 'participant_left':   return `${entry.participantName} left the chat`
    case 'topic_changed':      return `Topic changed to: ${entry.topicTitle}`
  }
}
```

`SystemEventEntry` has no `target` — it maps to `'public'` unconditionally.
